...
for companies but also for personal use, especially as home assistant. 
...
sorts inputs into \emph{classes}. For example, we can train a classifier to 
...
For non critical uses, we can simply test the model for several inputs 
to see if the model is able to recognize cats from dogs. But in sensitive 
domains, we may want to verify that the model effectively recognizes a cat 
instead of a wool ball or a cat tree in background. In this case, the model 
could effectively make an error and class a tumor scan in the benign scan class and then 
create a medical error. 

When we use non-complex models such as small Random Forest, we can 
verify each decision of the model during the inference process. But for 
more complex ones like Neural Networks with billions of parameters, the human
...
This document details the currently-used models and systems for classifier 
explanation.
% We start by presenting the SHAP-Score one which assign an importance 
...
We start by presenting a theoretical approach with Abductive Explanation methods. This section tries to formalize a classifier's reasoning to provide a formal  guarantee of the produced explanation. Then, we present methods based on Shapley values that assign an importance value to each input feature of a decision, based on game theory principles. Finally, we present perturbation methods with LIME and Anchor systems. We present the theoretical aspects of these methods and
their ameliorations.

% Table \ref{tab:comparaison_xai} provides a preliminary comparison of these 
% approaches to guide the reader through the following sections.
